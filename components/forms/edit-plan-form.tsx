...
export function EditPlanModal({ isOpen, onClose, onUpdated, plan }: EditPlanModalProps) {
  ...
  useEffect(() => {
    if (plan) {
      setFormData({
        ...
        coverage_details: plan.coverage_details ? JSON.stringify(plan.coverage_details, null, 2) : "",
  ...
  const updatePlanMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await fetch(`/api/underwriting/plans/${plan.id}`, {
        method: "PUT",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-blue-600">Edit Plan</DialogTitle>
          <DialogDescription>Update plan details and coverage information</DialogDescription>
        </DialogHeader>
