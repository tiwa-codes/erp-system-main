...
export function EditHRRuleForm({ 
  hrRule, 
  onSuccess, 
  onCancel 
}: { 
  ...
  // Initialize form with HR rule data
  useEffect(() => {
    if (hrRule) {
      setForm({
        name: hrRule.name || "",
        description: hrRule.description || "",
        rule_type: hrRule.rule_type || "",
        conditions: JSON.stringify(hrRule.conditions, null, 2),
        actions: JSON.stringify(hrRule.actions, null, 2),
        is_active: hrRule.is_active !== undefined ? hrRule.is_active : true,
        priority: hrRule.priority || 0
      })
    }
  }, [hrRule])
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      const res = await fetch(`/api/hr/rules/${hrRule.id}`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        ...
        throw new Error(error.message || 'Failed to update HR rule')
      }
      return res.json()
    },
    onSuccess: () => {
      toast({
        title: "Success",
        description: "HR Rule updated successfully",
      })
      onSuccess()
    },
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to update HR rule",
        variant: "destructive"
      })
    }
  })
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit" disabled={updateMutation.isPending}>
          {updateMutation.isPending ? "Updating..." : "Update HR Rule"}
        </Button>
      </div>
    </form>
  )
