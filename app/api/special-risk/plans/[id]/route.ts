...
const planUpdateSchema = z.object({
  ...
  // Allow updating limits, services, etc. via metadata
  metadata: z.any().optional(),
})

const looksLikeInternalId = (value: string) => /^[a-z0-9]{20,}$/i.test(String(value || "").trim())

async function findPlanByParam(rawParam: string, include?: any) {
  const rawId = String(rawParam || "").trim()
  const where = looksLikeInternalId(rawId)
...
export async function GET(
  ...
  try {
    ...
    const plan = await findPlanByParam(params.id, {
        created_by: {
          select: {
            id: true,
            first_name: true,
  ...
  try {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        action: "SPECIAL_RISK_PLAN_UPDATE",
        resource: "plan",
