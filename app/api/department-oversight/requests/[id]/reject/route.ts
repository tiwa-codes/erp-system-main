...
export async function PATCH(
  ...
  try {
    ...
    const canReject = await checkPermission(session.user.role as any, 'department-oversight', 'edit')
    if (!canReject) {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        action: 'PROCUREMENT_DEPARTMENT_REJECTED',
        resource: 'procurement_invoice',
        resource_id: id,
        old_values: procurementRequest,
        new_values: {
          ...updatedRequest,
          rejection_reason_note: reason,
        },
      }
    })
    ...
    console.error('Error rejecting department oversight request:', error)
    return NextResponse.json(
