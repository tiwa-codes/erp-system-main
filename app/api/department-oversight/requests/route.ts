...
export async function GET(request: NextRequest) {
  try {
    ...
    const canView = await checkPermission(session.user.role as any, 'department-oversight', 'view')
    if (!canView) {
    ...
    // Build where clause
    const where: any = {
      status: 'PENDING' // Only show pending requests for department oversight
    }
    ...
    console.error('Error fetching department oversight requests:', error)
    return NextResponse.json(
