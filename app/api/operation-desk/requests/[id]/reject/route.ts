...
export async function PATCH(
  ...
  try {
    ...
    const canReject = await checkPermission(session.user.role as any, 'operation-desk', 'edit', 'procurement-bill')
    if (!canReject) {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        action: 'PROCUREMENT_OPERATIONS_REJECTED',
    ...
    console.error('Error rejecting operation desk request:', error)
    return NextResponse.json(
