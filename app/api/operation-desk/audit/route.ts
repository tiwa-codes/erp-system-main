...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build where clause for audit - show all providers and their claim statistics
    const where: any = {
      current_stage: 'audit',
    ...
    if (status && status !== 'all' && status !== ClaimStatus.VETTER2_COMPLETED) {
    ...
    // Get all claims that have been completed by Vetter2 (for audit)
    ...
    if (provider && provider !== 'all') {
      // Re-fetch claims without status filter for detail pages
    ...
    const providerStats = allClaims.reduce((acc: any, claim) => {
      ...
      if (claim.status === 'VETTER2_COMPLETED') {
        stats.pending_audit++
      } else if (claim.status === 'AUDIT_COMPLETED' || claim.status === 'APPROVED') {
        stats.audited++
