...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get metrics for operation desk vetting
    const [
      totalPending,
      totalVetted,
      totalFlagged,
      ...
      prisma.claim.count({
        where: { status: { in: ['AUDIT_COMPLETED', 'APPROVED'] } }
      }),
      prisma.claim.count({
        where: { 
          status: 'VETTER2_COMPLETED',
          fraud_alerts: { some: { status: 'OPEN' } }
        }
      }),
      prisma.claim.aggregate({
        where: { status: { in: ['VETTER2_COMPLETED', 'AUDIT_COMPLETED', 'APPROVED', 'REJECTED'] } },
        _sum: { amount: true }
      }),
      prisma.claim.aggregate({
    ...
    console.error("Error fetching operation desk vetting metrics:", error)
    return NextResponse.json(
