  ...
  try {
    ...
    try {
      ...
      const serviceType = await prisma.serviceType.findFirst({
        where: {
          ...
          OR: [
            { service_category: 'Radiology Services' },
            { service_category: 'Radiology / Imaging' },
            { service_category: { contains: 'Radiology', mode: 'insensitive' } }
          ]
        }
      })
    ...
    await prisma.telemedicineRequest.create({
      data: {
        ...
        facility_id,
        test_name,
        description: `Radiology test request: ${test_name}`,
        status: 'PENDING',
    ...
    await prisma.auditLog.create({
      data: {
        ...
        action: "RADIOLOGY_ORDER_CREATE",
        resource: "radiology_order",
        resource_id: radiologyOrder.id,
        new_values: radiologyOrder
      }
    })

    // Send email notification to facility
    try {
      const publicLink = `${process.env.NEXTAUTH_URL}/public/radiology-results/${radiologyOrder.id}`
      const facilityPortalLink = `${process.env.NEXTAUTH_URL}/public/radiology-results/${radiologyOrder.id}`
      
      await notificationService.sendRadiologyOrderEmail(
        facility.email,
      ...
      // Don't fail the radiology order creation if email fails
    }
    ...
    return NextResponse.json({
      ...
      message: "Radiology order created successfully. Email sent to facility and EHR request created. Claim will be created when facility completes the order."
    }, { status: 201 })
