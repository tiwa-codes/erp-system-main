...
export async function GET(request: NextRequest) {
  try {
    ...
    const groupedMode = searchParams.get("grouped") === "providers"
    // 'auto' = provider-originated claims only
    // 'manual' = call-centre manual-code claims only
    // ''      = all (backward-compat)
    ...
    // Filter by current_stage = 'vetter1'
    whereClause.current_stage = 'vetter1'

    // Bill type filter — auto vs manual
    if (billType === 'manual') {
    ...
    if (status && status !== "all" && status !== "") {
      ...
      // Default to showing PENDING claims (newly submitted to vetter)
      whereClause.status = {
        in: ["NEW", "PENDING", "SUBMITTED", "UNDER_REVIEW", "VETTING"]
      }
    }

    // Get all claims for statistics (not filtered by status)
    ...
    if (provider && provider !== "all" && !groupedMode) {
      ...
      const filteredClaims = await prisma.claim.findMany({
        ...
        select: {
          ...
          // For manual-tab deleted-code visibility
          approval_codes: {
            select: {
              id: true,
              approval_code: true,
      ...
      const formattedClaims = await Promise.all(filteredClaims.map(async (claim) => {
        ...
        return {
          ...
          // Manual-code metadata for the Manual Bills tab
          code_deleted: (claim as any).approval_codes?.some((ac: any) => ac.is_manual && ac.is_deleted) ?? false,
          manual_code: (claim as any).approval_codes?.find((ac: any) => ac.is_manual)?.approval_code ?? null
        }
      }))
    ...
    // For overview pages, group claims by provider and calculate statistics
    // First, fetch facility names for all telemedicine claims
    ...
    const providerStats = claimsWithFacilityNames.reduce((acc: any, claim) => {
      ...
      if (!acc[providerId]) {
        acc[providerId] = {
          ...
          total_amount: 0,
          latest_claim: null,
          is_telemedicine: !claim.provider_id && (claim.claim_type?.startsWith('TELEMEDICINE_') || false)
        }
      }
      ...
      // Count by status
      if (["NEW", "PENDING", "SUBMITTED", "UNDER_REVIEW", "VETTING"].includes(claim.status)) {
        stats.pending_vetting++
      } else if (claim.status === 'VETTER1_COMPLETED' || claim.status === 'VETTER2_COMPLETED' ||
        claim.status === 'AUDIT_COMPLETED' || claim.status === 'APPROVED') {
        stats.vetted++
    ...
    // Filter claims based on provider and grouped-search fields shown in UI
    let filteredClaims = formattedClaims

    if (provider && provider !== "all") {
      filteredClaims = formattedClaims.filter(c => c.provider_id === provider)
    }

    if (search.trim()) {
      const q = search.trim().toLowerCase()
      filteredClaims = filteredClaims.filter((c: any) => {
        const providerName = (c.provider_name || "").toLowerCase()
        const latestClaimNumber = (c.latest_claim?.claim_number || "").toLowerCase()
        const latestEnrolleeName = (c.latest_claim?.enrollee_name || "").toLowerCase()
        const latestEnrolleeId = (c.latest_claim?.enrollee_id || "").toLowerCase()
        const statusText = (c.latest_claim?.status || "").toLowerCase()

        return (
          providerName.includes(q) ||
          latestClaimNumber.includes(q) ||
          latestEnrolleeName.includes(q) ||
          latestEnrolleeId.includes(q) ||
