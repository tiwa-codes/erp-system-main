...
export async function POST(
  ...
  try {
    ...
    // Get next stage
    const nextStage = getNextStage('vetter1')
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Update claim status to VETTER1_COMPLETED
      await tx.claim.update({
        where: { id: params.id },
        data: {
          status: ClaimStatus.VETTER1_COMPLETED,
          current_stage: nextStage,
          processed_at: new Date()
        }
      })

      // 🕒 TIMELINE TRACKING (VETTER1_COMPLETED)
      try {
        ...
        for (const code of approvalCodes) {
          // Find previous stage (CLAIM_SUBMITTED) to calculate delay
          const prevStage = await tx.approvalCodeTimeline.findFirst({
            where: { approval_code_id: code.id, stage: 'CLAIM_SUBMITTED' },
        ...
        console.error('❌ Failed to create Vetter 1 timeline entry:', timelineError)
      }
    })
    ...
    return NextResponse.json({
      success: true,
      message: "Claim approved and moved to Vetter 2"
    })
  } catch (error) {
    console.error("Error approving claim at Vetter 1:", error)
    return NextResponse.json(
