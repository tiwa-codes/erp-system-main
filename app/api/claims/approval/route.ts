...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check if user has approval permissions (MD/Admin)
    ...
    // Build where clause - Filter by current_stage = 'approval'
    ...
    // Get all claims for approval stage
    ...
    const dependentEnrolleeIds = allClaims
      .filter(c => c.enrollee_id && c.enrollee_id !== c.principal?.enrollee_id)
      .map(c => c.enrollee_id as string)
    const dependentMap: Record<string, { first_name: string; last_name: string; dependent_id: string }> = {}
    if (dependentEnrolleeIds.length > 0) {
      const deps = await prisma.dependent.findMany({
        where: { dependent_id: { in: dependentEnrolleeIds } },
        select: { dependent_id: true, first_name: true, last_name: true }
      })
      deps.forEach(d => { dependentMap[d.dependent_id] = d })
    }
    ...
    if (provider && provider !== "all") {
      const formattedClaims = await Promise.all(allClaims.map(async (claim) => {
        ...
        return {
          ...
          beneficiary: dependentMap[claim.enrollee_id] || null,
          provider: {
