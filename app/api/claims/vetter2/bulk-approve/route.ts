...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get all pending claims at vetter2 stage for this provider
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const claim of pendingClaims) {
        await createVettingAction(
          ...
          'APPROVED',
          session.user.id,
          comments || "Bulk approved by Vetter 2",
          undefined,
          tx
        )
        ...
        await tx.auditLog.create({
          data: {
            ...
            action: "CLAIM_VETTER2_BULK_APPROVED",
            resource: "claim",
            ...
            new_values: {
              status: ClaimStatus.VETTING_COMPLETED,
              current_stage: "audit"
            }
          }
        })
