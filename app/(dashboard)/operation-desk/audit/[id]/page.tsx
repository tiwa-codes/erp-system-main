...
export default function ProviderAuditPage() {
  ...
  const { data: claimsData, isLoading } = useQuery({
    queryKey: ["audit-provider-claims", id, currentPage],
    queryFn: async () => {
      const res = await fetch(`/api/operation-desk/audit?provider=${id}&page=${currentPage}&limit=${limit}`)
  ...
  // Count pending claims at audit stage
  const pendingClaims = claims.filter((c: any) =>
    c.status === 'VETTING_COMPLETED' || c.status === 'VETTER2_COMPLETED'
  )

  // Bulk approve mutation
  const bulkApproveMutation = useMutation({
    mutationFn: async (data: { provider_id: string; comments: string }) => {
      const res = await fetch('/api/operation-desk/audit/bulk-approve', {
        method: 'POST',
    ...
    onSuccess: (data) => {
      ...
      setBulkComments("")
      queryClient.invalidateQueries({ queryKey: ['audit-provider-claims', id] })
    },
    onError: (error: Error) => {
      toast({
        title: "Error",
  ...
  const confirmBulkApprove = () => {
    bulkApproveMutation.mutate({
      provider_id: id,
      comments: bulkComments || "Bulk approved by Auditor"
    })
  }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'VETTER2_COMPLETED':
        return <Badge className="bg-yellow-100 text-yellow-800"><Clock className="w-3 h-3 mr-1" /> Pending Audit</Badge>
      case 'AUDIT_COMPLETED':
        return <Badge className="bg-blue-100 text-blue-800"><CheckCircle className="w-3 h-3 mr-1" /> Audited</Badge>
      default:
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600">Internal Control audit claims for this provider</p>
        </div>
      </div>
                ...
                {claims.map((claim: any) => (
                  <TableRow key={claim.id}>
                    ...
                    <TableCell className="text-right">
                      <Button
                        size="sm"
                        onClick={() => router.push(`/operation-desk/audit/process/${claim.id}`)}
                      >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              This action will move all claims to MD Approval stage.
            </DialogDescription>
          </DialogHeader>
