...
export default function ExecutiveDeskPage() {
  ...
  const { data: requestsData, isLoading } = useQuery({
    queryKey: ["executive-desk-requests", currentPage, limit, debouncedSearchTerm, startDate, endDate, selectedDepartment, selectedStatus],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/executive-desk/requests?${params}`)
      if (!res.ok) {
  ...
  const approveRequestMutation = useMutation({
    mutationFn: async ({ requestId, comment }: { requestId: string, comment?: string }) => {
      const res = await fetch(`/api/executive-desk/requests/${requestId}/approve`, {
        method: 'PATCH',
  ...
  const rejectRequestMutation = useMutation({
    mutationFn: async ({ requestId, reason }: { requestId: string, reason: string }) => {
      const res = await fetch(`/api/executive-desk/requests/${requestId}/reject`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    <PermissionGate module="executive-desk" action="view">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Executive Desk</h1>
            <p className="text-gray-600">Manage executive services</p>
          </div>
        </div>
