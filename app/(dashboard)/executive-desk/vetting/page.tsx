...
export default function ExecutiveDeskVettingPage() {
  ...
  // Fetch claims data for approval vetting
  const { data: claimsData, isLoading } = useQuery({
    queryKey: ["executive-approval-vetting-claims", currentPage, limit, debouncedSearchTerm, selectedStatus, selectedProvider],
    queryFn: async () => {
  ...
  // Fetch vetting metrics for approval
  const { data: metricsData } = useQuery({
    queryKey: ["executive-approval-vetting-metrics"],
    queryFn: async () => {
  ...
  const { data: providersData } = useQuery({
    queryKey: ["executive-approval-vetting-providers"],
    queryFn: async () => {
  ...
  const vetClaimMutation = useMutation({
    mutationFn: async ({ claimId, action, findings }: { claimId: string, action: 'approve' | 'reject', findings?: string }) => {
      const res = await fetch(`/api/executive-desk/vetting/${claimId}`, {
        method: 'POST',
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["executive-approval-vetting-claims"] })
      queryClient.invalidateQueries({ queryKey: ["executive-approval-vetting-metrics"] })
      toast({
        title: "Success",
    ...
    <PermissionGate module="executive-desk" action="approve">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Executive Desk - Vetting</h1>
            <p className="text-gray-600">Executive vetting and final claim approval</p>
          </div>
          <PermissionGate module="executive-desk" action="approve">
            <Button 
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Claims awaiting executive approval
              </p>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Filter claims for executive vetting</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-4">
              ...
              <Select value={selectedStatus} onValueChange={setSelectedStatus}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="AUDIT_COMPLETED">Ready for Approval</SelectItem>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Executive Vetting Queue</CardTitle>
            <CardDescription>Claims ready for executive vetting</CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
