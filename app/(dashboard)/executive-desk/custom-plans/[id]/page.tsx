...
import { ArrowLeft, CheckCircle, Edit, XCircle, Download } from "lucide-react"
...
export default function ExecutiveDeskCustomPlanDetailPage({
  params,
}: {
  params: { id: string }
}) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["executive-desk-plan", params.id],
    queryFn: async () => {
      const res = await fetch(`/api/executive-desk/plans/${params.id}`)
  ...
  const approveMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/executive-desk/plans/${params.id}/approve`, {
        method: "POST",
      })
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["executive-desk-plan", params.id] })
      queryClient.invalidateQueries({ queryKey: ["executive-desk-plans"] })
      toast({
        title: "Success",
        description: "Plan approved and activated. It is now available for use.",
      })
      router.push("/executive-desk/custom-plans")
    },
    onError: (error: Error) => {
      toast({
        title: "Error",
  ...
  const rejectMutation = useMutation({
    mutationFn: async (reason: string) => {
      const res = await fetch(`/api/executive-desk/plans/${params.id}/reject`, {
        method: "POST",
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "Plan rejected and sent back to Special Services",
      })
  ...
  if (!plan) {
    return (
      <div className="text-center py-8">
        ...
        <Link href="/executive-desk/custom-plans">
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <PermissionGate module="executive-desk" action="view">
            <Button
              variant="outline"
              onClick={() => {
                window.location.href = `/api/executive-desk/plans/${params.id}/export`
              }}
            >
            ...
            <>
              <PermissionGate module="executive-desk" action="edit">
                <Button
                  ...
                  onClick={() =>
                    router.push(
                      `/underwriting/plans/${params.id}/customize?returnTo=/executive-desk/custom-plans/${params.id}&stageLabel=Executive%20Desk`
                    )
                  }
                >
      ...
      <div className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Special Services Approved By</p>
              <p className="font-medium">
            ...
            {plan.special_risk_approved_at && (
              <div>
                <p className="text-sm text-muted-foreground">Special Services Approved Date</p>
                <p className="font-medium">
      ...
      {!specialServiceConfig?.enabled && plan.customizations && plan.customizations.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Customized Categories & Services</CardTitle>
            <CardDescription>Reviewed customization passed from Underwriting through Special Services</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
      ...
      <Dialog open={isRejectOpen} onOpenChange={setIsRejectOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will send the plan back to Special Services for another review pass.
            </DialogDescription>
          </DialogHeader>
