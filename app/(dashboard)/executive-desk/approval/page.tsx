...
export default function ApprovalPage() {
  ...
  const { data: claimsData, isLoading } = useQuery({
    queryKey: ["executive-approval-claims", currentPage, limit, debouncedSearchTerm, selectedStatus, selectedProvider],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/executive-desk/approval?${params}`)
      if (!res.ok) {
  ...
  const processedClaims: ProviderStats[] = claims.map((provider: any) => ({
    ...
    pending_claims: provider.pending_approval,
    vetted_claims: provider.approved,
    ...
    <PermissionGate module="executive-desk" action="approve">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Executive Desk - Approval</h1>
            <p className="text-gray-600">Review and approve processed claims</p>
          </div>
          <div className="flex gap-2">
            ...
            <Button className="bg-green-600 hover:bg-green-700">
              <Download className="h-4 resize-4 mr-2" />
              Report
            </Button>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle>Executive Approval Overview {'>>'} {selectedProvider !== "all" ?
                  providersData?.providers?.find((p: any) => p.id === selectedProvider)?.facility_name || "All Providers" :
                  "All Providers"
                }</CardTitle>
                <CardDescription className="mt-2">Claims ready for executive approval</CardDescription>
              </div>
              <div className="flex gap-2">
                <Button className="bg-green-600 hover:bg-green-700">
                  <Download className="h-4 w-4 mr-2" />
                          ...
                          <TableCell>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent>
                                <DropdownMenuItem
                                  onClick={() => {
                                    router.push(`/executive-desk/approval/${provider.provider_id}`)
                                  }}
                                >
                                  View Enrollees
                                </DropdownMenuItem>
