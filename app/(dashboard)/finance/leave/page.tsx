...
export default function FinanceLeavePage() {
  ...
  const { data: leaveData, isLoading, refetch } = useQuery({
    queryKey: ["finance-leave-requests", selectedEmployee, startDate, endDate, currentPage],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/finance/leave?${params}`)
  ...
  const markAsPaidMutation = useMutation({
    mutationFn: async (leaveId: string) => {
      const res = await fetch(`/api/finance/leave/${leaveId}/mark-paid`, {
        method: 'PATCH'
      })
      if (!res.ok) {
    ...
    onSuccess: () => {
      toast({ title: "Leave request marked as paid successfully" })
      refetch()
    },
    onError: (error: Error) => {
      toast({ 
  ...
  const handleMarkAsPaid = async (leaveId: string) => {
    const leave = leaveData?.leave_requests?.find((l: any) => l.id === leaveId)
    if (!leave) return
    
    if (!confirm(`Mark leave request as paid for ${leave.employee?.first_name} ${leave.employee?.last_name}?`)) return
    
    try {
      await markAsPaidMutation.mutateAsync(leaveId)
    } catch (error) {
      console.error('Error marking leave as paid:', error)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Finance Leave Management</h1>
          <p className="text-gray-600">Manage leave requests and payments for the Finance module</p>
        </div>
      ...
      <Card className="bg-white">
        <CardHeader>
          <CardTitle>Finance Leave Requests</CardTitle>
        </CardHeader>
        <CardContent>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          {leave.status === 'APPROVED' && (
                            <DropdownMenuItem 
                              onClick={() => handleMarkAsPaid(leave.id)}
