...
interface Invoice {
  ...
  generated_by: string
  amount: number
  status: 'PENDING' | 'PENDING_OPERATIONS' | 'PENDING_MD' | 'PENDING_FINANCE' | 'APPROVED' | 'REJECTED' | 'PAID'
...
export default function ProcurementPage() {
  ...
  const resubmitInvoiceMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await fetch(`/api/hr/procurement/${data.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!res.ok) {
        ...
        throw new Error(error.error || 'Failed to resubmit invoice')
      }
      return res.json()
    },
    onSuccess: () => {
      toast({
        title: "Success",
        description: "Invoice updated and resubmitted successfully",
      })
      queryClient.invalidateQueries({ queryKey: ["procurement-invoices"] })
      setShowCreateModal(false)
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "Failed to resubmit invoice",
        variant: "destructive",
      })
    },
  })
  ...
  const rejectPaymentMutation = useMutation({
    mutationFn: async (data: { invoiceId: string; reason: string }) => {
      const res = await fetch(`/api/finance/procurement/${data.invoiceId}/reject-payment`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rejection_reason: data.reason })
      })
      if (!res.ok) {
  ...
  const handleCreateInvoice = () => {
    ...
    // Validate description is provided when service_type is "other"
    if (invoiceForm.service_type === "other" && !invoiceForm.description.trim()) {
      toast({
        title: "Error",
        description: "Please specify what you're requesting bill for",
        variant: "destructive",
      })
      return
    }
    ...
    if (editingInvoiceId) {
      resubmitInvoiceMutation.mutate({
        id: editingInvoiceId,
        payload: {
          ...invoiceForm,
          attachment: invoiceAttachment,
          resubmission_comment: invoiceForm.description,
        },
      })
      return
    }
                        ...
                        <TableCell className="text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {(invoice.status === 'PENDING_FINANCE' || invoice.status === 'APPROVED') && (
                                <DropdownMenuItem
            ...
            <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {selectedInvoice.rejection_reason && (
                    <div className="col-span-2">
                      ...
                      <p className="text-red-600 font-medium">{selectedInvoice.rejection_reason}</p>
                    </div>
                  )}
                  ...
                  {selectedInvoice.status === 'PAID' && (
                    <div>
                      ...
                      <p>{new Date(selectedInvoice.updated_at).toLocaleDateString('en-GB')}</p>
                    </div>
                  )}
                </div>

                {/* Verification Comments Section */}
                {(selectedInvoice.dept_oversight_comment || selectedInvoice.operations_comment || selectedInvoice.executive_comment) && (
                  <div className="border-t pt-4 mt-4">
                    <h3 className="text-blue-600 font-semibold mb-4">Verification Comments</h3>
                    <div className="space-y-4">
                      {selectedInvoice.dept_oversight_comment && (
                        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-semibold text-blue-600">Department Oversight</span>
                            {selectedInvoice.dept_oversight_at && (
                              <span className="text-xs text-gray-500">
                                {new Date(selectedInvoice.dept_oversight_at).toLocaleDateString('en-GB')}
                              </span>
                            )}
                          </div>
                          <p className="text-sm text-gray-700">{selectedInvoice.dept_oversight_comment}</p>
                          {selectedInvoice.dept_oversight_by && (
                            <p className="text-xs text-gray-500 mt-1">
                              By: {selectedInvoice.dept_oversight_by.first_name} {selectedInvoice.dept_oversight_by.last_name}
                            </p>
                          )}
                        </div>
                      )}
                      {selectedInvoice.operations_comment && (
                        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-semibold text-blue-600">Operations</span>
                            {selectedInvoice.operations_at && (
                              <span className="text-xs text-gray-500">
                                {new Date(selectedInvoice.operations_at).toLocaleDateString('en-GB')}
                              </span>
                            )}
                          </div>
                          <p className="text-sm text-gray-700">{selectedInvoice.operations_comment}</p>
                          {selectedInvoice.operations_by && (
                            <p className="text-xs text-gray-500 mt-1">
                              By: {selectedInvoice.operations_by.first_name} {selectedInvoice.operations_by.last_name}
                            </p>
                          )}
                        </div>
                      )}
                      {selectedInvoice.executive_comment && (
                        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-semibold text-blue-600">Executive</span>
                            {selectedInvoice.executive_at && (
                              <span className="text-xs text-gray-500">
                                {new Date(selectedInvoice.executive_at).toLocaleDateString('en-GB')}
                              </span>
                            )}
                          </div>
                          <p className="text-sm text-gray-700">{selectedInvoice.executive_comment}</p>
                          {selectedInvoice.executive_by && (
                            <p className="text-xs text-gray-500 mt-1">
                              By: {selectedInvoice.executive_by.first_name} {selectedInvoice.executive_by.last_name}
                            </p>
                          )}
                        </div>
                      )}
                ...
                <div className="flex justify-end gap-4 pt-4 border-t">
                  {(selectedInvoice.status === 'APPROVED' || selectedInvoice.status === 'PENDING_FINANCE') && (
        ...
        {showCreateModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  <span>{editingInvoiceId ? 'Edit & Resend Invoice' : 'Create New Invoice'}</span>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => {
                ...
                <CardDescription>{editingInvoiceId ? 'Update rejected invoice details and resend it for review' : 'Generate a new procurement invoice'}</CardDescription>
              </CardHeader>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-sm font-medium">
                        Description {invoiceForm.service_type === "other" ? "*" : "(optional)"}
                      </label>
                      <Input
                        placeholder={invoiceForm.service_type === "other" ? "What are you requesting bill for? *" : "Enter description"}
                        value={invoiceForm.description}
                        onChange={(e) => setInvoiceForm(prev => ({ ...prev, description: e.target.value }))}
                        className={invoiceForm.service_type === "other" && !invoiceForm.description.trim() ? "border-red-500" : ""}
                      />
                      {invoiceForm.service_type === "other" && !invoiceForm.description.trim() && (
                        <p className="text-xs text-red-500 mt-1">This field is required when selecting "Other"</p>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <Card className="w-full max-w-md">
              <CardHeader>
                ...
                <CardDescription>
                  Provide a reason for rejecting payment for invoice {selectedInvoice.invoice_number}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
