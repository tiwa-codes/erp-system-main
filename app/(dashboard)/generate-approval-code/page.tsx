...
export default function GenerateCodePage() {
  ...
  const fetchApprovalRequest = async (id: string) => {
    try {
      setIsLoading(true)
      const res = await fetch(`/api/request-from-provider/provider-requests/${id}`)
      if (!res.ok) {
  ...
  const approveRequestMutation = useMutation({
    mutationFn: async (approvalData: any) => {
      const res = await fetch(`/api/request-from-provider/provider-requests/${requestId}/approve`, {
        method: 'POST',
    ...
    onSuccess: (data) => {
      ...
      router.push('/request-from-provider')
    },
    onError: () => {
      toast({
        title: "Error",
  ...
  const rejectRequestMutation = useMutation({
    mutationFn: async (rejectionData: any) => {
      const res = await fetch(`/api/request-from-provider/provider-requests/${requestId}/reject`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <Button onClick={() => router.push('/request-from-provider')}>
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Request from Provider
          </Button>
        </div>
      </div>
    )
  ...
  return (
    <PermissionGate module="request-from-provider" action="approve">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button
              variant="outline"
              onClick={() => router.push('/request-from-provider')}
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Generate Approval Code</h1>
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {approvalForm.services.map((service: any) => {
                  ...
                  if (isAdHoc) {
                    ...
                    badge = <Badge className="bg-orange-500 text-white text-[10px] ml-2">Ad-hoc</Badge>;
                  } else if (isAddedLater) {
                    rowClassName = "bg-purple-50 hover:bg-purple-100";
                    badge = <Badge className="bg-purple-500 text-white text-[10px] ml-2">Added Later</Badge>;
                  }

                  return (
                    <TableRow key={service.id} className={rowClassName}>
