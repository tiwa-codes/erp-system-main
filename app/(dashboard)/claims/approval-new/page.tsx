...
export default function ApprovalPage() {
  ...
  const navigateToApprovalVetting = (claimId: string) => {
    const currentQuery = searchParams.toString()
    const returnTo = currentQuery ? `${pathname}?${currentQuery}` : pathname
    router.push(`/claims/approval-new/vetter/${claimId}?returnTo=${encodeURIComponent(returnTo)}`)
  }
  ...
  const getActionDropdown = (claim: Claim) => {
    return (
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          <DropdownMenuItem 
            onClick={() => navigateToApprovalVetting(claim.id)}
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle>Approval - Claims Processing</CardTitle>
                <CardDescription className="mt-2">Approve claims from Audit</CardDescription>
              </div>
                  ...
                  <TableBody>
                    {claims.map((claim: Claim) => (
                      <TableRow key={claim.id}>
                        ...
                        <TableCell>
                          {(claim.beneficiary || claim.principal) ? (
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 bg-red-100 rounded-full flex items-center justify-center">
                                <span className="text-sm font-medium text-red-400">
                                  {(claim.beneficiary?.first_name ?? claim.principal?.first_name)?.[0]}{(claim.beneficiary?.last_name ?? claim.principal?.last_name)?.[0]}
                                </span>
                              </div>
                              <div>
                                <button
                                  type="button"
                                  onClick={() => navigateToApprovalVetting(claim.id)}
                                  className="font-medium text-left text-blue-700 hover:text-blue-900 hover:underline"
                                >
                                  {claim.beneficiary
                                    ? `${claim.beneficiary.first_name} ${claim.beneficiary.last_name}`
                        ...
                        <TableCell>
                          <button
                            type="button"
                            onClick={() => {
                              setSelectedProvider(claim.provider_id)
                              setCurrentPage(1)
                            }}
                        ...
                        <TableCell>
                          <span className={`${getStatusBadgeColor(claim.status)} text-xs`}>
                            {claim.status === 'SUBMITTED' ? 'Pending' : 
                             ...
                             claim.status === 'APPROVAL' ? 'For Approval' :
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold">
                      {selectedClaim.beneficiary
                        ? `${selectedClaim.beneficiary.first_name} ${selectedClaim.beneficiary.last_name} (${selectedClaim.beneficiary.dependent_id})`
