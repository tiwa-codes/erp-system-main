...
export default function VetterPage() {
  ...
  const { data: claimsData, isLoading } = useQuery({
    queryKey: ["vetter2-claims", currentPage, limit, debouncedSearchTerm, selectedStatus, selectedProvider],
    queryFn: async () => {
  ...
  const { data: metricsData } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch("/api/claims/vetter/metrics")
      if (!res.ok) {
  ...
  const vetAllMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/claims/vetter/vet-all", {
        method: "POST",
  ...
  const handleVetClaim = (claim: Claim) => {
    ...
    router.push(`/claims/vetter2/vetter/${claim.id}?returnTo=${encodeURIComponent(returnTo)}`)
  }
    ...
    <PermissionGate module="claims" action="vet">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Vetter 2</h1>
            <p className="text-gray-600">Second-level claim review and validation</p>
          </div>
          <PermissionGate module="claims" action="vet">
            <Button 
                      ...
                      <TableRow key={claim.id}>
                        ...
                        <TableCell>
                          <span className={`${getStatusBadgeColor(claim.status)} text-xs`}>
                            {claim.status === 'VETTER1_COMPLETED' ? 'Pending Vetter 2' :
