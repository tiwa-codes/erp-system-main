...
export default function Vetter2Page() {
  ...
  const stage: 'vetter1' | 'vetter2' | 'audit' | 'approval' = 'vetter2'

  const navigateBack = () => {
    if (returnTo) {
      router.push(returnTo)
    } else {
      router.push('/claims/vetter2')
    }
  }
      ...
      for (const action of vettingActionsData.actions) {
        if (Array.isArray(action.service_verdicts)) {
          for (const sv of action.service_verdicts) {
            // Later actions overwrite earlier ones (array is ordered asc)
  ...
  const approveMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/claims/${claimId}/vetter2/approve`, {
        method: "POST",
    ...
    onSuccess: () => {
      toast.success("Claim approved and moved to Audit")
      clearDraft()
      navigateBack()
      queryClient.invalidateQueries({ queryKey: ["claim-details"] })
      queryClient.invalidateQueries({ queryKey: ["provider-claims-vetter2"] })
    },
    onError: (error: Error) => {
  ...
  const rejectMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/claims/${claimId}/vetter2/reject`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          comments: vettingComments,
    ...
    onSuccess: () => {
      toast.success("Claim rejected and returned to Vetter 1")
      clearDraft()
      navigateBack()
      queryClient.invalidateQueries({ queryKey: ["claim-details"] })
      queryClient.invalidateQueries({ queryKey: ["provider-claims-vetter2"] })
  ...
  const completedStages: any[] = ['vetter1']
  if (claim.current_stage !== 'vetter2' && claim.current_stage !== 'vetter1') completedStages.push('vetter2')

  const currentStage = (claim.current_stage || 'vetter2') as any
      ...
      <div className="space-y-6 pb-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Badge className="bg-emerald-600 hover:bg-emerald-700">STAGER 2</Badge>
                <div className="h-1.5 w-1.5 rounded-full bg-gray-300" />
                <p className="text-xs text-gray-400 font-bold tracking-widest uppercase">Clinical Review Desk</p>
                {lastAction ? (
                  <div className={`flex items-center gap-1.5 ml-4 px-2 py-0.5 border rounded text-[10px] font-bold uppercase tracking-tighter ${isRejectedBack ? "bg-red-50 border-red-100 text-red-600" : "bg-emerald-50 border-emerald-100 text-emerald-600"}`}>
                  ...
                  <div className="flex items-center gap-1.5 ml-4 px-2 py-0.5 bg-emerald-50 border border-emerald-100 rounded text-[10px] font-bold text-emerald-600 uppercase tracking-tighter">
                    <Clock className="h-3.5 w-3.5" />
                    RECEIVED FROM {claim.provider?.facility_name || 'PROVIDER'} AT {formatDateTime(claim.submitted_at)}
                  </div>
                )}
              </div>
              <h1 className="text-3xl font-black text-gray-900 tracking-tight">Vetting Approval: {claim.claim_number}</h1>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="border-emerald-200 shadow-sm overflow-hidden">
            <CardHeader className="py-3 bg-emerald-50/60 border-b">
            ...
            <CardContent className="pt-4">
              <div className="rounded-xl border border-emerald-100 bg-emerald-50/40 p-4 text-sm font-semibold text-gray-800 whitespace-pre-wrap">
                {clinicalDiagnosis}
              </div>
            </CardContent>
          </Card>
          ...
          <div>
            <PriceEditor
              ...
              onSave={() => {
                ...
                queryClient.invalidateQueries({ queryKey: ["vetter2-claims"] })
                queryClient.invalidateQueries({ queryKey: ["provider-claims-vetter2"] })
              }}
        ...
        <div className="space-y-10">
          ...
          <Card className="border-emerald-200 bg-emerald-50/40">
            <CardContent className="py-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-emerald-800">Subtotal (Services + Drugs)</span>
                <span className="text-xl font-black text-emerald-700">₦{liveTotal.toLocaleString()}</span>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="border-gray-200 shadow-sm overflow-hidden">
          <CardHeader className="py-3 bg-gray-50 border-b">
            <CardTitle className="text-xs font-black uppercase tracking-widest flex items-center gap-2 text-gray-500">
              ...
              Vetter 2 Clinical Assessment
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-4">
            <Textarea
              placeholder="Add your final clinical assessment before move to audit..."
              value={vettingComments}
              onChange={(e) => setVettingComments(e.target.value)}
              className="min-h-[140px] bg-white border-gray-200 focus:ring-emerald-500 focus:border-emerald-500 transition-all text-sm font-medium leading-relaxed"
            />
          </CardContent>
        </Card>
        ...
        <Card className="border-gray-200 shadow-sm overflow-hidden">
          ...
          <CardContent className="pt-4 space-y-3">
            ...
            <label className={`flex items-center justify-center gap-2 border-2 border-dashed rounded-lg p-4 cursor-pointer transition-all ${isUploadingFile ? "opacity-60 cursor-not-allowed border-gray-200" : "border-blue-200 hover:border-blue-400 hover:bg-blue-50/30"}`}>
              ...
              <span className="text-sm text-blue-600 font-medium">{isUploadingFile ? "Uploading..." : "Click to attach a file"}</span>
        ...
        {/* Action Section */}
        <Card className="border-2 border-emerald-600 bg-emerald-600 text-white shadow-xl">
          <CardContent className="py-6">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="space-y-1">
                <h4 className="font-black text-xl">Finalize Clinical Review</h4>
                <p className="text-emerald-100 text-sm font-medium">Approved claims proceed to the final multi-level audit stage</p>
              </div>
        ...
        <Dialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader className="gap-2">
              ...
              <DialogTitle className="text-xl font-bold">Reject at Stage 2?</DialogTitle>
              <DialogDescription>
                This will send the claim back to Vetter 1 or return to provider depending on policy.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4">
          ...
          <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto p-0 border-none shadow-2xl">
            <div className="sticky top-0 z-10 bg-white border-b p-6">
              <DialogHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <Badge variant="outline" className="mb-2 border-emerald-200 text-emerald-700 bg-emerald-50 font-bold">CLINICAL HISTORY</Badge>
            ...
            <div className="p-8 space-y-8 bg-gray-50/30">
              ...
              <div className="bg-white rounded-2xl p-6 border shadow-sm">
                ...
                <div className="text-lg font-bold text-gray-800 bg-emerald-50/30 p-4 rounded-xl border border-emerald-100">
            ...
            <div className="p-6 bg-white border-t flex justify-end">
              <Button onClick={() => setShowEncountersModal(false)} className="rounded-full px-8 font-bold bg-emerald-600 hover:bg-emerald-700 text-white border-none shadow-lg">CLOSE VIEW</Button>
            </div>
          </DialogContent>
        </Dialog>
