...
export default function AuditPage() {
  ...
  const navigateToAuditVetting = (claimId: string) => {
    const currentQuery = searchParams.toString()
    const returnTo = currentQuery ? `${pathname}?${currentQuery}` : pathname
    router.push(`/claims/audit-new/vetter/${claimId}?returnTo=${encodeURIComponent(returnTo)}`)
  }
  ...
  const getActionDropdown = (claim: Claim) => {
    return (
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          <DropdownMenuItem 
            onClick={() => navigateToAuditVetting(claim.id)}
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle>Audit - Claims Processing</CardTitle>
                <CardDescription className="mt-2">Audit claims from Vetter 2</CardDescription>
              </div>
                      ...
                      <TableRow key={claim.id}>
                        ...
                        <TableCell>
                          <button
                            type="button"
                            onClick={() => {
                              setSelectedProvider(claim.provider.id)
                              setCurrentPage(1)
                            }}
                            className="text-left text-blue-700 hover:text-blue-900 hover:underline"
                          >
                        ...
                        <TableCell>
                          {claim.status === 'AUDIT' ? '1' : '0'}
                        </TableCell>
                        <TableCell>
                          {claim.status === 'APPROVAL' ? '1' : '0'}
                        </TableCell>
                        <TableCell>
