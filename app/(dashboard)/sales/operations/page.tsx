  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `/api/sales/reports?submodule=${SalesSubmodule.SALES_OPERATIONS}&limit=1000`
      )
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Manage procurement reports and activities</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-2">
            <Link href="/sales/operations/reports/add?type=DAILY">
              <Button variant="outline" className="w-full justify-start">
                Submit Daily Report
              </Button>
            </Link>
            <Link href="/sales/operations/reports/add?type=WEEKLY">
              <Button variant="outline" className="w-full justify-start">
                Submit Weekly Report
              </Button>
            </Link>
            <Link href="/sales/operations/reports/add?type=MONTHLY">
              <Button variant="outline" className="w-full justify-start">
                Submit Monthly Report
              </Button>
            </Link>
            <Link href="/sales/operations/reports">
              <Button variant="outline" className="w-full justify-start">
                View All Reports
              </Button>
            </Link>
            ...
            {isLoading ? (
              ...
              <div className="space-y-2">
                {recentReports.map((report: any) => (
                  <Link
                    key={report.id}
                    href={`/sales/operations/reports/${report.id}`}
