...
export default function EditCorporateSalesReportPage({ params }: { params: { id: string } }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["corporate-sales-report", params.id],
    queryFn: async () => {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push(`/sales/corporate/reports/${params.id}`)
    },
    onError: (error: Error) => {
      toast({
        title: "Error",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/sales/corporate/reports/${params.id}`}>
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Public Sector Channel Report</h1>
