...
export default function EditAgencySalesReportPage({ params }: { params: { id: string } }) {
  const router = useRouter()
  const { toast } = useToast()

  const { data, isLoading } = useQuery({
    queryKey: ["agency-sales-report", params.id],
    queryFn: async () => {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push(`/sales/agency/reports/${params.id}`)
    },
    onError: (error: Error) => {
      toast({
        title: "Error",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/sales/agency/reports/${params.id}`}>
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Institutional Business & Digital Sales Report</h1>
