...
export default function SpecialRisksSalesDashboard() {
  const { data: stats, isLoading } = useQuery({
    queryKey: ["special-risks-sales-stats"],
    queryFn: async () => {
      const res = await fetch(
        `/api/sales/reports?submodule=${SalesSubmodule.SPECIAL_RISKS_SALES}&limit=1000`
      )
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Special Service Sale</h1>
        <p className="text-muted-foreground">Manage special service sale reports and activities</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-2">
            <Link href="/sales/special-risks/reports/add?type=DAILY">
              <Button variant="outline" className="w-full justify-start">
                Submit Daily Report
              </Button>
            </Link>
            <Link href="/sales/special-risks/reports/add?type=WEEKLY">
              <Button variant="outline" className="w-full justify-start">
                Submit Weekly Report
              </Button>
            </Link>
            <Link href="/sales/special-risks/reports/add?type=MONTHLY">
              <Button variant="outline" className="w-full justify-start">
            ...
            {isLoading ? (
              ...
              <div className="space-y-2">
                {recentReports.map((report: any) => (
                  <Link
                    key={report.id}
                    href={`/sales/special-risks/reports/${report.id}`}
