...
export default function EditSpecialRisksSalesReportPage({ params }: { params: { id: string } }) {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push(`/sales/special-risks/reports/${params.id}`)
    },
    onError: (error: Error) => {
      toast({
        title: "Error",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/sales/special-risks/reports/${params.id}`}>
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Special Service Sale Report</h1>
